Public Class FrmPreGIExtra
    ...
    Private Sub cmdImport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdImport.Click
        ...
        If String.IsNullOrEmpty(txtOrder.Text) Then MsgBox("กรุณาใส่ Header") : txtOrder.Focus() : Exit Sub
        If String.IsNullOrEmpty(txtVersion.Text) Then MsgBox("กรุณาใส่ Version") : txtVersion.Focus() : Exit Sub


        Dim Date1 As Date = cx.ExecuteScalar("select xDate from tbt_600GiToConsoh where Headertext = '" & HeaderText & "'")
        If Date1 = Nothing Then
            ...
            If cdl.ShowDialog = Windows.Forms.DialogResult.OK Then
                Try
                    ...
                    cmd = cx.CommandCreate("truncate table tbt_600preGiToConsoh")
                    cx.Execute(cmd)

                    For r As Integer = 0 To dtgItem.Rows.Count - 1
                        Try
                            cmd = cx.CommandCreate("insert into tbt_600preGiToConsoh (HeaderText,itemNo,Productcode,OrderQty,Extrajob) values (?,?,?,?,?)", "TITIT")
                    ...
                    DV = Nothing
                    dtgItem.DataSource = Nothing
                    DT = cx.GetdataTable("select a.*,c.baseunit, b.GlAccount , b.CostCenter from tbt_600preGiToConsoh A left join tbm_600GiConsohAccount B " & _
                     "on a.productcode = B.productcode left join [172.18.8.188].npiconsohpd.dbo.tbm_product C on a.productcode = c.productcode ")
                    ...
                    For r As Integer = 0 To dtgItem.Rows.Count - 1
                        ...
                        ElseIf dtgItem.Item("OrderQty", r).Value = 0 Then
                            ...
                            cmdSave.Enabled = False
                            Exit Sub
                        ElseIf dtgItem.Item("Extrajob", r).Value.ToString.Count <> 11 Then
                            MsgBox("รหัส Extra job ไม่ถูกต้อง")
                            dtgItem.Rows(r).DefaultCellStyle.BackColor = Color.Red
                            cmdSave.Enabled = False
                            Exit Sub
                        ElseIf Microsoft.VisualBasic.Left(dtgItem.Item("Extrajob", r).Value.ToString, 5) <> "87007" Then
                            MsgBox("รหัส Extra job ไม่ถูกต้อง")
                            dtgItem.Rows(r).DefaultCellStyle.BackColor = Color.Red
                            cmdSave.Enabled = False
                            Exit Sub
        ...
        For r As Integer = 0 To dtgItem.Rows.Count - 1
            Try
                Dim cmd As OleDbCommand = cx.CommandCreate("insert into tbt_600GiToConsoh " & _
                    "(Uname,HeaderText,itemNo,Productcode,OrderQty,GiinusedOrderno,unit,GLaccount,Costcenter) " & _
                    " values (?,?,?,?,?,?,?,?,?)", "TTITITTTT")
                ...
                cmd.Parameters(5).Value = "0" & dtgItem.Item("Extrajob", r).Value
                cmd.Parameters(6).Value = dtgItem.Item("baseunit", r).Value
                cmd.Parameters(7).Value = dtgItem.Item("GLaccount", r).Value
                cmd.Parameters(8).Value = dtgItem.Item("Costcenter", r).Value
